...
const assignItem = async (req, res) => {
  ...
  if (isNaN(userId) || isNaN(articleId)) return res.status(400).json({ message: "ID must be a number" });
  try {
    const findUser = await User.findByPk(userId);
    if (findUser) {
      const findArticle = await Article.findByPk(articleId);
      if (!findArticle) {
        return res.status(404).json({ message: "An article with that ID could not be found" });
      } else {
...
const deleteItem = async (req, res) => {
  ...
  try {
    ...
    if (findWished) {
      ...
      return res.status(200).json({ message: "Article deleted from wishlist successfully" });
    } else {
